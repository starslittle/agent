...
from rag.retrievers.hybrid_retriever import HybridRetriever, CrossEncoderReranker
...
BACKEND_ROOT = API_DIR.parents[1]
...
def load_prompt_template(path: str) -> ChatPromptTemplate:
    ...
    if p.is_absolute():
        ...
        candidate_paths = [
            ...
            (API_DIR / p),
            (BACKEND_ROOT / p),
            (BACKEND_ROOT / "prompts" / p.name),
            (REPO_ROOT / p),
            (REPO_ROOT / "backend" / p),
            (REPO_ROOT / "backend" / "prompts" / p.name),
        ]
        if p.parent == Path(""):
            candidate_paths.extend([
                API_DIR / "prompts" / p.name,
                BACKEND_ROOT / "prompts" / p.name,
                REPO_ROOT / "prompts" / p.name,
            ])
...
def load_tool(name: str):
    ...
    if key == "get_lunar_chart":
        return get_lunar_chart

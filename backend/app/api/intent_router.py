...
# 意图分类的提示词模板
INTENT_CLASSIFICATION_PROMPT = load_prompt(
...
class IntentRouter:
    ...
    def classify_intent_with_trace(
        self, user_input: str
    ) -> tuple[Literal["chat", "task"], dict[str, Any] | None]:
        """
        分类用户输入的意图
        ...
        if len(user_input) > 20:  # 中等长度的文本才需要LLM判断
            prompt_path = "agent/prompts/intent_classification.txt"
            rendered_prompt = INTENT_CLASSIFICATION_PROMPT.replace(
    ...
    def classify_intent(self, user_input: str) -> Literal["chat", "task"]:
        """Compatibility wrapper for callers that do not consume prompt traces."""
        intent, _ = self.classify_intent_with_trace(user_input)
        return intent

...
def main() -> None:
    ...
    try:
        ...
        print("[STEP] 非流式调用测试…")
        llm = ChatTongyi(model="deepseek-v4-flash", temperature=0.2, dashscope_api_key=key, streaming=False)
    ...
    try:
        from langchain_community.chat_models import ChatTongyi
        print("[STEP] 流式调用测试…")
        llm2 = ChatTongyi(model="deepseek-v4-flash", temperature=0.2, dashscope_api_key=key, streaming=True)
        got = False
